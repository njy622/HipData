...
def get_restaurant_list(place):
    ...
    data = []
    for li in lis:
        name = li.select_one('h2').get_text()
        score = li.select_one('.score').get_text()
        ...
        addr = sub_soup.select('.pc_only>td')[0].get_text().split('지번')[0].strip()
        tel = sub_soup.select('.pc_only>td')[1].get_text().strip()       
        data.append({'업소명':name, '평점':score, '메뉴':menu, '주소':addr, '전화번호':tel, '이미지':img, 'href':href})
